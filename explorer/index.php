...
/****************************************************************************/
/*                                                                          */
/*   Encode Explorer  6.3                   繁體中文翻譯 by 如果 ids93216   */
/*                                                                          */
/*  >本程式多語言部分僅保留英文及繁體中文，需其他語言請自行翻譯或回官網查。 */
/*  >本程式之繁中翻譯僅前台顯示及設定之註解部分，翻譯不好請見諒。           */
/*  >本程式僅供學術研究使用，請不要用來做營利等違法之用途。                 */
/*  >請做個有品的人，尊重原作者及譯者，使用或轉載請務必保留這段文字，謝謝！ */
/*                                                                          */
/*   祝您使用愉快！              魚樂出品 http://fishome.tw/   2013.08.18   */
/*                                                                          */
/****************************************************************************/
/*  >備註：修正1479行原本為mktime為time，部分主機會報錯。                   */
/****************************************************************************/
 ...
 *             Encode Explorer
 *
 *             Author / Autor : Marek Rei (marek at siineiolekala dot net)
 *
 *             Version / Versioon : 6.3
 *
 *             Last change / Viimati muudetud : 23.09.2011
 *
 *             Homepage / Koduleht : http://encode-explorer.siineiolekala.net/
 *
 *             繁體中文翻譯 / 魚樂小格 : http://fishome.tw/
 *
 *
 ...
 *                  voivad probleemid tekkida, eriti piltide kuvamisega.
 *
 *             NB!：細部的註解都是英文的。
 *                  設定值的註解是愛沙尼亞文、英文和繁體中文。
 *                  如果你更改了任何地方，請盡量使用UTF-8！
 *                  否則你可能會遇到問題，尤其是在顯示圖片時。
 *
 ***************************************************************************/
...
//
// Algvaartustame muutujad. Ara muuda.
//
...
//
// Vali sobiv keel. Allpool on naha voimalikud valikud. Vaikimisi: en
//
// Choose a language. See below in the language section for options.
//
// 語言設定，已經設定成繁體中文。
//
//預設： $_CONFIG['lang'] = "zh_TW";
$_CONFIG['lang'] = "zh_TW";

//
// Kuva pildifailidele eelvaated. Vaikimisi: true
// Levinumad pildifailide tuubid on toetatud (jpeg, png, gif).
...
//
// 當游標聽在圖片上面時顯示縮圖。
// 主流的格式都有支援（JPG、PNG、GIF）。
// 也支援PDF檔，但空間需要支援ImageMagick。
//
//預設： $_CONFIG['thumbnails'] = true;
//
...
// 縮圖的尺寸限制（單位：像素）。
//
//預設： $_CONFIG['thumbnails_width'] = 200;
//預設： $_CONFIG['thumbnails_height'] = 200;
//
...
// 開啟手機模式。
//
//預設： $_CONFIG['mobile_enabled'] = true;
//
$_CONFIG['mobile_enabled'] = false;

//
// Mobiilidele moeldud kasutajaliides avaneb automaatselt.
// Vaikimisi: false
//
// Mobile interface as the default setting.
//
// 預設使用手機模式（包含電腦也是）。
//
//預設： $_CONFIG['mobile_default'] = false;
//
...
// 在新視窗開啟檔案。
//
//預設： $_CONFIG['open_in_new_window'] = false;
//
...
// 統計所有檔案佔用的空間。
//
//預設： $_CONFIG['calculate_space_level'] = false;
//
$_CONFIG['calculate_space_level'] = true;

//
// Kas kuvatakse lehe pais?
//
...
// 設定標題的名稱‧。
//
//預設： $_CONFIG['main_title'] = "Encode Explorer";
//
$_CONFIG['main_title'] = "雲林縣莿桐國民中學-106學年度課程資料網站";

//
// Pealkirjad, mida kuvatakse lehe paises, suvalises jarjekorras.
//
// The secondary page titles, randomly selected and displayed under the main header.
//
// 設定第二標題。
//
//預設： $_CONFIG['secondary_titles'] = array();
//
$_CONFIG['secondary_titles'] = array();

//
// Kuva asukoht kataloogipuus.
//
// Display breadcrumbs (relative path of the location).
//
// 顯示相對路徑。
//
//預設： $_CONFIG['show_path'] = true;
//
...
// 顯示網頁載入時間。
//
//預設： $_CONFIG['show_load_time'] = true;
//
...
//
// 最後修改時間的格式，已設定為台灣之使用習慣。
//
//預設： $_CONFIG['time_format'] = "Y-m-d H:i:s";
//
...
// Naiteks: CONFIG['hidden_dirs'] = array("ikoonid", "kaustanimi", "teinekaust");
//
// The array of folder names that will be hidden from the list.
//
// 隱藏的資料夾
// 範例：CONFIG['hidden_dirs'] = array("lalala", "hahaha", "wahaha");
//
//預設： $_CONFIG['hidden_dirs'] = array();
//
$_CONFIG['hidden_dirs'] = array();

//
// Failide varjamine. Failide nimed mida lehel ei kuvata.
// NB! Margitud nimega failid ja kaustad varjatakse koigis alamkaustades.
//
// Filenames that will be hidden from the list.
//
// 隱藏的檔案。
//
//預設： $_CONFIG['hidden_files'] = array(".ftpquota", "index.php", "index.php~", ".htaccess", ".htpasswd", ".log.txt");
//
$_CONFIG['hidden_files'] = array(".ftpquota", "index.php", "index.php~", ".htaccess", ".htpasswd", ".log.txt", "robots.txt", "Thumbs.db", "index.jpg");

//
// Maara kas lehe nagemiseks peab sisse logima.
// 'false' tahendab, et leht on avalik
// 'true' tahendab, et kasutaja peab sisestama parooli (vaata allpool).
//
...
// 是否需要登入？
// 如果是false，不必登入就可以開啟。
// 如果是true，則需要登入或輸入密碼（請看下面）。
//
//預設： $_CONFIG['require_login'] = false;
//
$_CONFIG['require_login'] = false;

//
// Kasutajanimed ja paroolid, lehele ligipaasu piiramiseks.
// Naiteks: $_CONFIG['users'] = array(array("user1", "pass1"), array("user2", "pass2"));
// Voimalik lehte kaitsta ka ainult uldise parooliga.
// Naiteks: $_CONFIG['users'] = array(array(null, "pass"));
// Kui uhtegi kasutajat margitud ei ole, siis parooli ei kusita.
//
...
// -2.普通會員權限，但只需要密碼。
//  範例： $_CONFIG['users'] = array(array(null, "pass"));
// -3.同時有普通會員和管理員，需要帳號和密碼，管理員後面加admin，普通會員後面加user。
//  範例： $_CONFIG['users'] = array(array("username1", "password1", "user"), array("username2", "password2", "admin"));
// -4.一般人不需要登入，但允許管理員登入。※注意請關閉require_login！
//  範例： $_CONFIG['users'] = array(array("username", "password", "admin"));
// -5.僅允許訪客觀看，array() 保持預設值即可。※注意請關閉require_login！
//
//預設： $_CONFIG['users'] = array();
//
$_CONFIG['users'] = array();
...
// Naiteks: $_CONFIG['upload_dirs'] = array("./myuploaddir1/", "./mydir/upload2/");
// Kausta asukoht peab olema maaratud pohikausta suhtes, algama "./" ja loppema "/" margiga.
...
//
// 禁止上傳的資料夾。
// 範例：$_CONFIG['upload_dirs'] = array("./hahaha/", "./yo/hohoho/");
// 若要使用相對路徑，前面加"./"。記得最後要加斜線。
// 若本欄留空，則每個資料夾都有上傳的權限。
//
//預設： $_CONFIG['upload_dirs'] = array();
//
$_CONFIG['upload_dirs'] = array();

//
// MIME failituubid mis on uploadimiseks lubatud.
// Naiteks: $_CONFIG['upload_allow_type'] = array("image/png", "image/gif", "image/jpeg");
//
...
//
// 「允許」上傳的檔案類型，預設不限制。
// 例如，想要設定只能上傳圖片檔，應該設定成這樣：
// $_CONFIG['upload_allow_type'] = array("image/png", "image/gif", "image/jpeg");
//
//預設： $_CONFIG['upload_allow_type'] = array();
//
...
//
// 禁止上傳的檔案類型。
// 例如： $_CONFIG['upload_reject_extension'] = array("php", "html", "htm");
//
//預設： $_CONFIG['upload_reject_extension'] = array();
//
$_CONFIG['upload_reject_extension'] = array("php", "html", "htm");
...
// Kui maaratud, siis saadetakse sellele aadressile e-mail iga kord kui keegi lisab uue faili voi kausta.
//
// Upload notification e-mail.
// If set, an e-mail will be sent every time someone uploads a file or creates a new dirctory.
//
// 每次有人上傳檔案、新增資料夾時以Email通知。
//
//預設： $_CONFIG['upload_email'] = "";
//
$_CONFIG['upload_email'] = "";

//
// Logifail. Kui maaratud, siis kirjutatakse kaustade ja failide avamise kohta logi faili.
// Naiteks: $_CONFIG['log_file'] = ".log.txt";
//
...
//
// 記錄檔名稱，只要有人開啟檔案或資料夾就會被記錄。留空不紀錄。
// 範例： $_CONFIG['log_file'] = ".log.txt"; ※記得檔名前加一個點，這樣檔案才會被隱藏！
//
//預設： $_CONFIG['log_file'] = "";
//
$_CONFIG['log_file'] = "";
...
// 範例： $_CONFIG['starting_dir'] = "./mysubdir/";
//
//預設： $_CONFIG['starting_dir'] = ".";
//
...
//
// 本檔在伺服器裡的位置。一般不需更改，本程式會自動偵測。
// 因為某些伺服器的限制，部分使用者需要手動設定，這裡不多介紹。
//
//預設： $_CONFIG['basedir'] = "";
//
$_CONFIG['basedir'] = "";
...
//
// Suured failid. Kui sul on failiruumis vaga suured failid (>4GB), siis on see vajalik
// faili suuruse oigeks maaramiseks. Vaikimisi: false
//
// Big files. If you have some very big files (>4GB), enable this for correct
// file size calculation.
//
// 大型檔案。如果你有大於4G的檔案，開啟這個選項以便精準的計算正確的大小。
//
//預設： $_CONFIG['large_files'] = false;
//
...
//
// Session名稱，作為cookie使用的名字。
// 如果你在這個空間裡有使用多個程式，並希望他們登入狀態可以分開，
// 或是本程式的Session跟其他支程式的打架（Session重疊），那請為每支程式設定不同的值。
// ※只能是英文或數字
//
//預設： $_CONFIG['session_name'] = "";
//
...
$_TRANSLATIONS["zh_TW"] = array(
	...
	"password" => "密碼",
	"upload" => "上傳檔案",
	"failed_upload" => "檔案上傳失敗",
	"failed_move" => "檔案無法移動到正確的資料夾",
	"wrong_password" => "密碼錯誤",
	"make_directory" => "新資料夾",
	...
	"location" => "位置",
	"root" => "根資料夾",
	"log_file_permission_error" => "本程式沒有權限寫入log檔",
...
/*          繁體中文翻譯到這裡結束，祝您使用愉快！                         */
/*                                                                         */
/*                               魚樂出品 http://fishome.tw/   2013.06.29  */
/***************************************************************************/
...
/*   Naiteks siin: http://www.motobit.com/util/base64-decoder-encoder.asp  */
/*   Voi siin: http://www.greywyvern.com/code/php/binary2base64            */
/*   Voi kasuta lihtsalt PHP base64_encode() funktsiooni                   */
/*                                                                         */
...
<div id="top">
	...
	<a href="<?php print $this->makeLink(false, false, null, null, null, ""); ?>">
		<span><?php if(EncodeExplorer::getConfig('main_title') != null) print EncodeExplorer::getConfig('main_title'); ?></span>
	</a>
<?php 
